...
// Production /predict handler. One Gemini request per user scan: this is
// important because the current project quota is only 5 requests/minute.
...
async function diagnose(req,res){
  ...
  try{
    ...
    if(status===401||status===403||/api.?key|unauthorized|permission denied|authentication/i.test(text)){
      return res.status(503).json({success:false,retryable:false,error:"Gemini API authentication failed. Check GEMINI_API_KEY and project permissions in Render."});
    }
    if(status===429||/resource.?exhausted|rate.?limit|quota/i.test(text)){
      return res.status(200).json({success:false,retryable:true,error:"Gemini rate limit reached (5 requests/minute). Please wait about 15–30 seconds before scanning again."});
    }
    if(status===503||/overloaded|temporarily unavailable|service unavailable/i.test(text)){
      return res.status(200).json({success:false,retryable:true,error:"Gemini is temporarily busy. Please wait 10–15 seconds and scan again."});
    }
    if(status===404||/model.?not.?found|not found.*model/i.test(text)){
      return res.status(503).json({success:false,retryable:false,error:`Gemini model '${model}' is unavailable. Set GEMINI_FAST_MODEL to a valid vision-capable Gemini model in Render.`});
    }
    if(e?.code==="TIMEOUT"){
      return res.status(200).json({success:false,retryable:true,error:"Gemini analysis timed out. Please hold the leaf steady and try again."});
    }

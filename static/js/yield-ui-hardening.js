/* Final historical-yield UI. Replaces the old humidity/index form and guarantees no undefined/not-estimated placeholders. */
(() => {
  const VERSION="2026-09-10-trained-yield-v3";
  ...
  function install(){
    ...
    if(!out)return false;
    const old=out.parentElement;
    if(!old)return false;
    old.querySelectorAll("#trainedYieldInputs,#yieldModelButton").forEach(x=>x.remove());
    const legacy=["yieldHumidityInput","yieldRainInput"];
    legacy.forEach(id=>document.getElementById(id)?.closest(".form-group")?.remove());
    ...
    out.innerHTML="<div style='opacity:.75'>Enter farm details and click Estimate Crop Yield.</div>";
    return true;
  }
  async function predictYield(){
    if(!document.getElementById("trainedYieldInputs")&& !install())return;
    const g=id=>document.getElementById(id)?.value;
    const p={state:g("yieldState"),crop:g("yieldCrop"),season:g("yieldSeason"),year:Number(g("yieldYear")),area:Number(g("yieldArea")),rainfall:Number(g("yieldRainfallModel")),fertilizer:Number(g("yieldFertilizerModel")),pesticide:Number(g("yieldPesticideModel"))};
    const out=document.getElementById("yieldResult"),btn=document.getElementById("yieldModelButton");
    if(!p.state||!p.crop||!p.season||!Number.isFinite(p.year)||p.year<2000||p.year>2100||!Number.isFinite(p.area)||p.area<=0||!Number.isFinite(p.rainfall)||p.rainfall<0||!Number.isFinite(p.fertilizer)||p.fertilizer<0||!Number.isFinite(p.pesticide)||p.pesticide<0){out.innerHTML="<div style='color:#fecaca'>Please enter valid values in all fields.</div>";return;}
    btn.disabled=true;btn.textContent="Calculating…";out.innerHTML="<div>Calculating historical yield estimate…</div>";
    try{
      const r=await fetch("/predict_yield",{method:"POST",headers:{"Content-Type":"application/json"},credentials:"same-origin",body:JSON.stringify(p)});const d=await r.json().catch(()=>({}));
      if(!r.ok||d.success!==true||!Number.isFinite(Number(d.yield_tpha)))throw Error(d.error||"Yield prediction failed.");
      const y=Number(d.yield_tpha),total=y*p.area,m=d.model_test_metrics||{};
      const metric=Number.isFinite(Number(m.mae_tpha))&&Number.isFinite(Number(m.r2))?`Validation: MAE ${Number(m.mae_tpha).toFixed(2)} t/ha · R² ${Number(m.r2).toFixed(3)}`:"Validation metrics unavailable";
      out.innerHTML=`<div style="padding:14px;border-radius:12px;border:1px solid rgba(52,211,153,.35);background:rgba(16,185,129,.08)"><div style="font-size:1.2rem;font-weight:700;color:var(--accent-green)">Estimated Yield: ${y.toFixed(3)} t/ha</div><div style="margin-top:7px">Estimated production: <b>${total.toFixed(3)} tonnes</b> for ${p.area.toFixed(2)} ha</div><div style="margin-top:8px;font-size:.82rem;opacity:.82">${esc(d.model||"Historical yield model")} · ${metric}</div><div style="margin-top:7px;font-size:.76rem;opacity:.7">Historical-data estimate. Production is excluded from model inputs to prevent target leakage.</div></div>`;
    }catch(e){out.innerHTML=`<div style="color:#fecaca;padding:12px">${esc(e.message||"Unable to calculate yield.")}</div>`;}finally{btn.disabled=false;btn.textContent="Estimate Crop Yield";}
  }

...
export async function testConnection(){if(!db)return;try{await Promise.race([getDocFromServer(doc(db,"_health","probe")),new Promise((_,reject)=>setTimeout(()=>reject(new Error("Probe timeout")),4000))]);console.log("✅ Firestore connection validated");}catch(error){console.log("ℹ️ Firestore connection probe unavailable; this does not block normal reads/writes.");}}
async function initFirebase(){try{const response=await fetch("/firebase_config",{credentials:"same-origin"});if(!response.ok)throw new Error(`Firebase configuration endpoint returned HTTP ${response.status}`);const cfg=await response.json();const missing=["apiKey","authDomain","projectId","appId"].filter(k=>!cfg?.[k]);if(missing.length)throw new Error(`Firebase configuration is incomplete: ${missing.join(", ")}`);app=initializeApp(cfg);auth=getAuth(app);const dbId=cfg.firestoreDatabaseId||"(default)";try{db=initializeFirestore(app,{experimentalForceLongPolling:true},dbId);}catch(_){db=getFirestore(app,dbId);}console.log(`✅ Firebase initialized (Firestore database: ${dbId})`);return{app,auth,db};}catch(err){app=null;auth=null;db=null;console.error("❌ Firebase initialization failed:",err.message);return{app:null,auth:null,db:null,error:err}}}

...
async function initFirebase(){try{const response=await fetch("/firebase_config",{credentials:"same-origin",cache:"no-store"});if(!response.ok)throw new Error(`Firebase configuration endpoint returned HTTP ${response.status}`);const cfg=await response.json();const missing=["apiKey","authDomain","projectId","appId"].filter(k=>!cfg?.[k]);if(missing.length)throw new Error(`Firebase configuration is incomplete: ${missing.join(", ")}`);app=initializeApp(cfg);auth=getAuth(app);const dbId=String(cfg.firestoreDatabaseId||"(default)").trim()||"(default)";try{db=initializeFirestore(app,{experimentalForceLongPolling:true},dbId);}catch(_){db=getFirestore(app,dbId);}console.log(`✅ Firebase initialized (Firestore database: ${dbId})`);return{app,auth,db};}catch(err){app=null;auth=null;db=null;console.error("❌ Firebase initialization failed:",err.message);return{app:null,auth:null,db:null,error:err}}}
